document.addEventListener('DOMContentLoaded', () => {
  ...
  function showToast(message, type='info') {
    ...
    const cont = document.getElementById('toasts') || (function(){
      const c = document.createElement('div'); c.id = 'toasts'; document.body.appendChild(c); return c;
    })();
    cont.appendChild(t);
  ...
  // Ensure buyer name modal on first visit
  function ensureBuyerName(){
    return new Promise((resolve)=>{
      let name = localStorage.getItem('buyerName') || '';
      if (name && name.trim()){ resolve(name); return; }
      // create modal
      const nm = document.createElement('div'); nm.id='nameModal'; nm.className='modal show';
      nm.innerHTML = `<div class="modal-content" style="max-width:420px;padding:18px">
        <h3>Введіть ваше ім'я</h3>
        <p>Щоб робити замовлення та переглядати їх, введіть ім'я.</p>
        <input id="guestNameInput" class="input" placeholder="Ім'я" style="margin-top:8px"/>
        <div style="text-align:right;margin-top:12px"><button id="guestNameBtn" class="btn-primary">Зберегти</button></div>
      </div>`;
      document.body.appendChild(nm);
      document.getElementById('guestNameBtn').addEventListener('click', ()=>{
        const v=document.getElementById('guestNameInput').value.trim();
        if (!v){ alert("Введіть ім'я"); return; }
  ...
  // My Orders: bind to bottom nav or create floating button fallback
  function ensureMyOrdersUI(){
    ...
    const btn=document.createElement('button'); btn.id='myOrdersBtn'; btn.className='btn-secondary'; btn.style.position='fixed'; btn.style.right='18px'; btn.style.bottom='80px'; btn.style.zIndex=10005; btn.textContent='Мої замовлення'; document.body.appendChild(btn);
    btn.addEventListener('click', openMyOrdersModal);
  }

  async function openMyOrdersModal(){
    const name = localStorage.getItem('buyerName');
    if (!name){ showToast("Вкажіть ім'я спочатку", 'error'); return; }
    ...
    const list = modal.querySelector('#myOrdersList'); list.innerHTML='<p>Завантаження...</p>';
    try{
      const res = await fetch('/api/myorders?name=' + encodeURIComponent(name));
      const data = await res.json();
      if (!Array.isArray(data)) throw new Error('Invalid');
      if (data.length === 0){ list.innerHTML='<p>Замовлень немає</p>'; return; }
      list.innerHTML='';
      data.forEach(o=>{
        const el=document.createElement('div'); el.className='order'; el.style.marginBottom='8px';
        el.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center"><strong>' + escapeHtml(o.item.title) + ' × ' + o.quantity + '</strong><div style="color:var(--muted)">' + escapeHtml(o.status) + '</div></div>' + '<div class="order-meta">' + new Date(o.createdAt).toLocaleString() + (o.cancellationReason?(' — Причина: ' + escapeHtml(o.cancellationReason)):'') + '</div>';
        if (o.status === 'pending'){
          const cancelBtn=document.createElement('button'); cancelBtn.className='btn-secondary'; cancelBtn.textContent='Відмінити'; cancelBtn.style.marginTop='8px';
          cancelBtn.addEventListener('click', ()=>{
            const reason = prompt('Причина відміни (опціонально):') || 'Не вказано';
            fetch('/api/orders/' + encodeURIComponent(o.id) + '/cancel', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ reason })})
              .then(r=>r.json()).then(resp=>{ if (resp.error) { showToast('Не вдалося: '+resp.error,'error'); return; } showToast('Замовлення відмінено','info'); openMyOrdersModal(); }).catch(e=>{ showToast('Мережна помилка','error'); });
          });
      ...
      list.innerHTML='<p>Помилка завантаження</p>';
    }
  }

  function escapeHtml(s){ return String(s).replace(/[&<>"']/g, (m)=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m])); }

  async function fetchItems(){ try{ const res = await fetch('/api/items'); items = await res.json(); render(); }catch(e){ console.error(e); showToast('Не вдалось завантажити товари','error'); } }
  ...
  function openOrderModal(item){
    ...
    modal.querySelector('#confirmOrder').addEventListener('click', async ()=>{
      ...
      try{
        ...
        if (!latestItem){ showLoading(false); showToast('Товар більше не доступний','error'); return; }
        if (latestItem.stock < q){ showLoading(false); showToast('Недостатньо на складі. Доступно: ' + latestItem.stock + ' шт','error'); return; }
        ...
        showLoading(false);
        if (!data.success){ showToast(data.error || 'Помилка','error'); return; }
        showToast('✅ Замовлення прийнято','success');
        setHeaderNick(name);
        modal.classList.remove('show');
        fetchItems();
      }catch(err){ showLoading(false); console.error(err); showToast('Мережна помилка','error'); }
    });
  }
